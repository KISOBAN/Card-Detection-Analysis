...
def getlevel(img):
    ...
    cv.imshow('Level',cropped)
    gray = cv.cvtColor(cropped, cv.COLOR_BGR2GRAY)
    gray = cv.bilateralFilter(gray,5,150,300)
    cv.imshow('Bilateral Filtered Image',gray)
...
#*************************************************************************************************  
    ...
    for i in range(8):
        ...
        ret[i],thresh[i] = cv.threshold(gray[i], 127, 255, cv.THRESH_BINARY) 
        #change to THRESH_TRUNC FOR SOME LIGHT COLORED IMAGES SUCH AS WIND
    ...
    plt.subplot(2,3,2)
    plt.title("Stock Dark")
    plt.imshow(result[0],'gray',vmin=0,vmax=255)
    
    plt.subplot(2,3,3)
    plt.title("Stock Light")
    plt.imshow(result[1],'gray',vmin=0,vmax=255)
    
    plt.subplot(2,3,4)
    plt.title("Stock Water")
    plt.imshow(result[2],'gray',vmin=0,vmax=255)
    
    plt.subplot(2,3,5)
    plt.title("Stock Earth")
    plt.imshow(result[3],'gray',vmin=0,vmax=255)
    
    plt.subplot(2,3,6)
    plt.title("Stock Fire")
    plt.imshow(result[4],'gray',vmin=0,vmax=255)
